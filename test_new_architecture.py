...
async def test_branch_workflow():
    ...
    branch = BranchControlNode(
        "branch",
        "分支",
        lambda x: "high" if x["score"] > 50 else "low"
    )
    high_path = TaskNode("high", "高分", lambda x: {"result": "优秀", "score": x["score"]})
    low_path = TaskNode("low", "低分", lambda x: {"result": "需要改进", "score": x["score"]})
