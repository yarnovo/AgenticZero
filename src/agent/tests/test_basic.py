...
def test_llm_config_creation():
    """测试LLM配置创建。"""
    config = LLMSettings(provider="openai", api_key="test-key", model="gpt-4")
...
def test_mcp_server_config_creation():
    """测试MCP服务器配置创建。"""
    config = MCPServerSettings(name="test-server", command="echo", args=["hello"])
...
def test_agent_config_creation():
    """测试智能体配置创建。"""
    llm_settings = LLMSettings(provider="openai", api_key="test-key", model="gpt-4")

    config = AgentSettings(name="test-agent", llm_settings=llm_settings)
