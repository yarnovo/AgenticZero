...
class GraphProxy:
    ...
    def __init__(self, graph: Graph | None = None, auto_validate: bool = True):
        """
        初始化图代理
    ...
    def get_edge(self, from_id: str, to_id: str, action: str = "default") -> Edge | None:
        """获取边对象"""
        edge = self._graph.get_edge(from_id, action)
    ...
    def from_dict(
        cls,
        data: dict[str, Any],
        node_factory: dict[str, type[BaseNode]] | None = None
    ) -> "GraphProxy":
        """
        从字典创建图代理
    ...
    def add_sequence(self, node_ids: list[str], node_types: list[str] | None = None) -> bool:
        """
        添加一个节点序列
