...
from graph import (
    Graph, BaseNode, GraphExecutor,
    SimpleNode, DataProcessorNode, ConditionalNode,
    RandomChoiceNode, LoggingNode, DelayNode, ErrorNode
)
...
async def test_random_choice_graph():
    ...
    choice = RandomChoiceNode("choice", "随机选择", {
        "path_a": 0.6,  # 60% 概率
        "path_b": 0.3,  # 30% 概率
        "path_c": 0.1   # 10% 概率
    })
...
async def main():
    ...
    await test_basic_graph()
    await test_conditional_graph()
    await test_data_processing_graph()
    await test_random_choice_graph()
