...
from .core import Graph, BaseNode, Edge, NodeStatus
...
class GraphSnapshot:
    ...
    def capture_execution(self, executor: GraphExecutor):
        ...
        if executor.context:
            self.execution_state = {
                ...
                "start_time": executor.context.start_time.isoformat() if executor.context.start_time else None,
    ...
    def _extract_custom_state(self, node: BaseNode) -> Dict[str, Any]:
        ...
        if hasattr(node, 'conversation_history'):
            custom_state['conversation_history'] = node.conversation_history
        if hasattr(node, 'retry_count'):
            custom_state['retry_count'] = node.retry_count
        if hasattr(node, 'circuit_state'):
            custom_state['circuit_state'] = node.circuit_state
...
class EnhancedGraph(Graph):
    ...
    def save_snapshot(self, snapshot: GraphSnapshot, filepath: Optional[Path] = None):
        """保存快照到文件"""
        if filepath is None:
    ...
    @classmethod
    def deserialize(cls, data: Dict[str, Any], node_factory: Optional[Dict[str, type]] = None) -> 'EnhancedGraph':
        """反序列化图结构
...
class ResumableExecutor(GraphExecutor):
    ...
    async def _continue_execution_from(self, node_id: str) -> ExecutionContext:
        ...
        while execution_queue and not self.is_paused:
            ...
            # 创建检查点
            if self.graph.auto_checkpoint and len(self.context.visited_nodes) % self.graph.checkpoint_interval == 0:
    ...
    async def execute_with_checkpoints(
        ...
        initial_input: Optional[Any] = None,
        start_node_id: Optional[str] = None,
        checkpoint_callback: Optional[Callable[[GraphSnapshot], None]] = None
    ) -> ExecutionContext:
        """带检查点的执行
        ...
        if self.graph.auto_checkpoint:
            snapshot = self._create_checkpoint("initial")
            if checkpoint_callback:
                await self._call_hook(checkpoint_callback, snapshot)
        ...
        try:
            ...
            # 图完成钩子
            for hook in self._on_graph_complete_hooks:
            ...
            snapshot = self._create_checkpoint("error")
            snapshot.context_data["error"] = str(e)
            snapshot.context_data["error_type"] = type(e).__name__
            if checkpoint_callback:
                await self._call_hook(checkpoint_callback, snapshot)
            raise