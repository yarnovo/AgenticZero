...
class ExecutionContext:
    ...
    def add_execution(self, node_id: str, result: Any, action: str, error: Optional[str] = None):
        """记录节点执行信息"""
...
class GraphExecutor:
    ...
    async def execute(self, start_node_id: Optional[str] = None) -> ExecutionContext:
        """
        执行图
        ...
        try:
            while self.current_node_id and iterations < self.max_iterations:
                ...
                try:
                    ...
                    # 执行后钩子
                    await self._run_hooks("after_node", node=node, action=action, context=self.context)
                    
                    # 获取下一个节点
                    next_node_id = self.graph.get_next_node_id(self.current_node_id, action)
                    
                    if next_node_id:
                        logger.info(f"从 {self.current_node_id} 转移到 {next_node_id}，动作: '{action}'")
                        self.current_node_id = next_node_id
                    else:
                        logger.info(f"没有找到动作 '{action}' 对应的下一个节点，执行结束")
    ...
    async def execute_parallel_branches(self, start_node_ids: List[str]) -> Dict[str, ExecutionContext]:
        """
        并行执行多个分支
...
class ConditionalExecutor(GraphExecutor):
    ...
    def __init__(self, graph: Graph, conditions: Dict[str, Callable], max_iterations: int = 100):
        """
        初始化条件执行器
            ...
            while self.current_node_id and iterations < self.max_iterations:
                ...
                if condition_name:
                    ...
                    if not should_execute:
                        logger.info(f"由于条件 {condition_name} 不满足，跳过节点 {self.current_node_id}")
                        node.status = NodeStatus.SKIPPED
                        
                        # 寻找跳过时的下一个节点
                        skip_next = self.graph.get_next_node_id(self.current_node_id, "skip")
                        if not skip_next:
                            skip_next = self.graph.get_next_node_id(self.current_node_id, "default")
                ...
                try:
                    ...
                    # 尝试错误处理路径
                    error_next = self.graph.get_next_node_id(self.current_node_id, "error")
                    if error_next:
