...
class ForkControlNode(AtomicControlNode):
    ...
    def __init__(
        self, node_id: str, name: str | None = None, fork_count: int = 2, **kwargs
    ):
        """初始化分叉控制节点
    ...
    async def exec(self) -> Any:
        """执行分叉逻辑"""
        input_data = self._get_input_data()
        return {"__fork__": True, "count": self.fork_count, "data": input_data}
...
class JoinControlNode(AtomicControlNode):
    ...
    def __init__(
        ...
        node_id: str,
        name: str | None = None,
        join_func: Callable[[list], Any] | None = None,
        **kwargs,
    ):
        """初始化汇聚控制节点
    ...
    async def exec(self) -> Any:
        ...
        if isinstance(input_data, list):
            ...
            if len(self.join_inputs) >= self.expected_count:
                ...
                return {
                    "__waiting__": True,
                    "collected": len(self.join_inputs),
