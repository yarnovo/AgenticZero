...
from graph import (
    Graph, GraphExecutor,
    DataProcessorNode, ConditionalNode, LoggingNode
)
...
async def example_tax_calculation():
    ...
    # 验证输入
    validate = DataProcessorNode(
        "validate", 
        "验证输入",
        processor_func=validate_amount
    )
    validate.metadata["input_data"] = 1000  # 示例输入
    
    # 计算税费
    calculate = DataProcessorNode(
        "calculate",
        "计算税费",
        processor_func=calculate_tax
    )
...
async def example_approval_workflow():
    ...
    # 各级审批
    manager_approval = LoggingNode(
        "manager",
        "经理审批",
        "✓ 经理已批准"
    )
    
    director_approval = LoggingNode(
        "director",
        "总监审批",
        "✓ 总监已批准"
    )
    
    cfo_approval = LoggingNode(
        "cfo",
        "财务总监审批",
        "✓ 财务总监已批准"
    )
    ...
    # 结果节点
    approved = LoggingNode(
        "approved",
        "批准",
        "✓ 审批通过！"
    )
    
    rejected = LoggingNode(
        "rejected",
        "拒绝",
        "✗ 审批被拒绝"
    )
    ...
    # 添加节点到图
    for node in [start, amount_check, dept_check, manager_approval, 
                 director_approval, cfo_approval, approved, rejected]:
