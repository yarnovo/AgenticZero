...
class Edge:
    ...
    def __init__(self, from_id: str, to_id: str, action: str = "default", weight: float = 1.0):
        """
        初始化边
...
class Graph:
    ...
    def add_edge(self, from_id: str, to_id: str, action: str = "default", weight: float = 1.0) -> None:
        """
        添加边到图中
    ...
    def remove_node(self, node_id: str) -> None:
        ...
        for edges_dict in self.edges.values():
            to_remove = [action for action, edge in edges_dict.items() if edge.to_id == node_id]
    ...
    def get_edge(self, from_id: str, action: str = "default") -> Optional[Edge]:
        """获取边"""
        return self.edges.get(from_id, {}).get(action)
    
    def get_next_node_id(self, current_id: str, action: str = "default") -> Optional[str]:
        """根据当前节点和动作获取下一个节点ID"""
    ...
    def validate(self) -> Tuple[bool, List[str]]:
        ...
        if self.start_node_id:
            for node_id in self.nodes:
                if node_id != self.start_node_id and not self.has_path(self.start_node_id, node_id):
