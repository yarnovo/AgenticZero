...
async def main():
    ...
    # 1. 创建新图
    print("1. 创建新的工作流图...")
    workflow = manager.create(name="data_pipeline", description="数据处理管道示例")
    print(f"   ✓ 创建成功: {workflow.name}")
    print("   - 文件保存在: demo_graphs/data_pipeline.yaml")
    print("   - 已自动加载到内存")
        ...
        for node_id, node in nodes:
            if isinstance(node, TaskNode):
                ...
                if node_id == "load_data":
                    node.process_func = lambda x: {"data": [1, 2, 3, 4, 5]}
                elif node_id == "clean_data":
                    node.process_func = lambda x: {
                        "data": [i * 2 for i in x.get("data", [])]
                    }
                elif node_id == "analyze_data":
                    node.process_func = lambda x: {
                        "mean": sum(x.get("data", [])) / len(x.get("data", [])),
                        "data": x.get("data", []),
                    }
                elif node_id == "save_results":
                    node.process_func = (
                        lambda x: f"分析完成，平均值: {x.get('mean', 0)}"
                    )
                else:
                    node.process_func = lambda x=None: x
