...
class SimulatedTask(TaskNode):
    ...
    def __init__(self, node_id: str, name: str, duration: float = 1.0, fail_rate: float = 0.0):
...
async def main():
    ...
    # 创建会失败的任务
    unreliable_task = SimulatedTask("unreliable", "不可靠任务", duration=0.5, fail_rate=0.7)
    ...
    # 添加节点
    exception_nodes = [start2, retry_task, timeout_task, try_catch, circuit_breaker, end2]
    ...
    print("执行异常处理工作流...")
    try:
        context2 = await executor2.execute_with_checkpoints(initial_input="exception_test")
