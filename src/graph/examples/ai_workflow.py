...
async def main():
    ...
    # AI分析节点
    analyzer = AIAnalyzer(
        "analyzer", "数据分析", analysis_type="trend_analysis", agent=agent
    )

    # AI路由节点：根据分析结果选择处理策略
    router = AIRouter(
        "router", "智能路由", routes=["optimize", "expand", "monitor"], agent=agent
    )

    # 不同策略的处理节点
    optimize = AIGenerator(
        "optimize", "优化方案生成", generation_type="plan", agent=agent
    )

    expand = AIGenerator("expand", "扩容方案生成", generation_type="plan", agent=agent)

    monitor = SequenceControlNode(
        "monitor", "监控设置", lambda x: {"action": "设置监控告警", "data": x}
    )
    ...
    # 结束节点
        ...
        "end", "完成", lambda x: {"final_result": x, "status": "completed"}
    )
