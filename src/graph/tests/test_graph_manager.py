...
class TestGraphManager:
    ...
    def test_load_graph(self):
        """测试加载图"""
        # 先创建但不自动加载
        proxy1 = self.manager.create("workflow2", auto_load=False)
    ...
    async def test_run_graph(self):
        ...
        if isinstance(start_node, TaskNode):
            start_node.execute_func = lambda: "Hello"
        
        end_node = proxy.get_node("end")
        if isinstance(end_node, TaskNode):
            end_node.execute_func = lambda x: f"{x} World!"
        
        # 运行
        result = await self.manager.run("executable", "Test")
        # 注意：实际结果依赖于执行器的实现
