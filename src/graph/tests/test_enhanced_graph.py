...
class TestEnhancedGraph:
    ...
    def test_graph_serialization(self):
        ...
        node2 = BranchControlNode(
            "branch1", "分支1", lambda x: "high" if x > 5 else "low"
        )
    ...
    def test_graph_deserialization(self):
        ...
        data = {
            ...
            "start_node": "n1",
            "end_nodes": ["n2"],
            "metadata": {"is_resumable": True, "checkpoint_interval": 10},
        }

        # 提供节点工厂
        node_factory = {"SequenceControlNode": SequenceControlNode}
...
class TestResumableExecutor:
    ...
    async def test_execute_with_checkpoints(self):
        ...
        context = await executor.execute_with_checkpoints(
            initial_input=5, checkpoint_callback=checkpoint_callback
        )
    ...
    async def test_error_checkpoint(self):
        ...
        try:
            await executor.execute_with_checkpoints(
                initial_input="test", checkpoint_callback=checkpoint_callback
            )
        except ValueError:
            pass
