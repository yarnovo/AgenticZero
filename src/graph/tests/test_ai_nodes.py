...
class MockAgent(IAgent):
    ...
    async def think(
        ...
        return AgentResponse(content=content, confidence=0.9, reasoning="模拟推理过程")
    ...
    async def evaluate(
        ...
        return {"scores": scores, "average": 0.8, "feedback": "良好"}
...
class TestAIAnalyzer:
    ...
    def test_analyzer_creation(self):
        """测试分析节点创建"""
        agent = MockAgent()
        node = AIAnalyzer("analyzer1", "数据分析", analysis_type="trend", agent=agent)
    ...
    async def test_analyzer_execution(self):
        """测试分析节点执行"""
        agent = MockAgent()
        node = AIAnalyzer("analyzer2", "执行分析", analysis_type="pattern", agent=agent)
...
class TestAIGenerator:
    ...
    def test_generator_creation(self):
        """测试生成节点创建"""
        node = AIGenerator("gen1", "代码生成", generation_type="code")
    ...
    async def test_generator_code(self):
        """测试代码生成"""
        agent = MockAgent()
        node = AIGenerator("gen2", "生成代码", generation_type="code", agent=agent)
    ...
    async def test_generator_plan(self):
        """测试计划生成"""
        agent = MockAgent()
        node = AIGenerator("gen3", "生成计划", generation_type="plan", agent=agent)
...
class TestAIEvaluator:
    ...
    def test_evaluator_creation(self):
        """测试评估节点创建"""
        criteria = ["质量", "性能", "可维护性"]
        node = AIEvaluator("eval1", "代码评估", criteria=criteria)
    ...
    async def test_evaluator_execution(self):
        ...
        agent = MockAgent()
        node = AIEvaluator(
            "eval2", "执行评估", criteria=["完整性", "准确性"], agent=agent
        )
...
class TestAIControlNodes:
    ...
    async def test_ai_router(self):
        ...
        node = AIRouter("router1", "智能路由", routes=routes, agent=agent)
    ...
    async def test_ai_planner(self):
        """测试AI规划器"""
        agent = MockAgent()
        node = AIPlanner("planner1", "智能规划", agent=agent)
    ...
    async def test_ai_planner_sub_graph_execution(self):
        """测试AI规划器子图执行"""
        agent = MockAgent()
        node = AIPlanner("planner2", "子图执行", agent=agent)
...
class TestAgentProxy:
    ...
    async def test_agent_proxy_evaluate(self):
        ...
        evaluation = await proxy.evaluate("测试对象", ["标准1", "标准2"])
