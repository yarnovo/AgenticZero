...
class TestTryCatchNode:
    ...
    async def test_try_catch_exception(self):
        ...
        node = TryCatchNode(
            "try4",
            "异常处理",
            try_func=failing_func,
            catch_func=error_handler
        )
    ...
    async def test_handle_exception_method(self):
        ...
        # 处理匹配的异常
        result = await node.handle_exception(
            ValueError("test"),
            {"input": "data"}
        )
        assert result["handled"] is True
...
class TestRetryNode:
    ...
    async def test_retry_success_first_attempt(self):
        ...
        node = RetryNode(
            "retry2",
            "首次成功",
            target_func=lambda x: {"success": x}
        )
        node._input_data = "test"
