...
class GraphFileManager:
    ...
    def create(self, name: str, config: dict[str, Any]) -> Path:
        ...
        # 添加 schema 引用
        config = {"$schema": "./graph-config.schema.json", **config}
...
class GraphManager:
    ...
    def create(
        self, name: str, description: str = "", auto_load: bool = True
    ) -> GraphProxy:
        """
        创建新图
        ...
        config = {
            ...
            "end_nodes": ["end"],
            "nodes": [
                {"id": "start", "type": "TaskNode", "name": "开始节点"},
    ...
    async def run(
        self, name: str, input_data: Any = None, auto_load: bool = True
    ) -> Any:
        """
        运行图
